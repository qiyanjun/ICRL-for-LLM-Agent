#!/usr/bin/env python3
"""
Analyze attention patterns of Llama4-Maverick-17B model on large ICRL contexts.
This script loads the model, processes a long prompt, and identifies
which positions receive the most attention.

Note: Llama4-Maverick-17B-128E requires access approval. 
To use it:
1. Visit https://huggingface.co/meta-llama/Llama-4-Maverick-17B-128E
2. Request access
3. Once approved, update the model_name in main()
"""
...
def analyze_attention_patterns(attention_matrix, tokens, top_positions):
    ...
    position_groups = {
        'start': float(last_token_attention[:seq_len//10].mean()),
        'early': float(last_token_attention[seq_len//10:seq_len//4].mean()),
        'middle': float(last_token_attention[seq_len//4:3*seq_len//4].mean()),
        'late': float(last_token_attention[3*seq_len//4:9*seq_len//10].mean()),
        'end': float(last_token_attention[9*seq_len//10:].mean())
    }
...
def main():
    ...
    # Model options:
    # - "meta-llama/Llama-4-Maverick-17B-128E" (requires access, supports 1M tokens)
    # - "meta-llama/Llama-4-Maverick-17B-128E-Instruct" (requires access, supports 1M tokens)
    # - "gpt2-large" (open model for testing, 1024 tokens)
    # - "EleutherAI/gpt-j-6B" (open model, 2048 tokens)
    # - "Qwen/Qwen3-32B" (32B model with good long context support)
    
    model_name = "Qwen/Qwen3-32B"  # Using Qwen3-32B
    
    # Set max_length based on model
    if "Llama-4-Maverick" in model_name:
        max_length = 131072  # Start with 128K for Llama4-Maverick (can go up to 1M)
    elif "gpt2" in model_name:
        max_length = 1024
    elif "gpt-j" in model_name:
        max_length = 2048
    elif "Qwen3-32B" in model_name:
        max_length = 2048  # Start very conservatively with 2K to avoid OOM
    else:
    ...
    print(f"Using LEFT truncation - keeping the LAST {max_length} tokens (most recent trials)")
    ...
    try:
        ...
        print(f"Memory error: {str(e)[:100]}...")
        print("Trying with reduced sequence length...")
        ...
        max_length = max_length // 4  # More aggressive reduction
        inputs = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=max_length)
        input_ids = inputs["input_ids"]
    ...
    print(f"Total tokens analyzed: {len(tokens)} (last tokens of the prompt)")
    ...
    output_prefix = f"attention_{model_name.replace('/', '_')}"
    visualize_attention(attention_matrix, top_positions, tokens, output_prefix)
    
    # Save results
    print("\nSaving results...")
    save_results(top_positions, tokens, analysis_results, output_prefix)
    
    print("\nAnalysis complete!")
    print(f"\nTo use Llama4-Maverick-17B:")
    print("1. Visit https://huggingface.co/meta-llama/Llama-4-Maverick-17B-128E")
    print("2. Request access and wait for approval")
    print("3. Update model_name in this script to 'meta-llama/Llama-4-Maverick-17B-128E'")
